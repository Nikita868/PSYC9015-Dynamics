function [t,x] = ForcedDampedMassSpring(x0,v0,b,k,beta,fw,stime,plotflag)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function [t x] = ForcedDampedMassSpring(x0,v0,b,w,beta,fw,stime,plotflag)
%
%  Syntax:
...
xlabel('x(1)');
ylabel('x(2)');
title(['Forced Damped Mass Spring [m=1, b=', sprintf('%.2f', b), ';  w=',  sprintf('%.2f', k),';  fw=',  sprintf('%.2f', fw),']']);

% calculate solutions for x0s
options = odeset('InitialStep',H_INC,'MaxStep',H_INC,'RelTol',realmax,'AbsTol',[realmax realmax realmax]);
[t x]=ode45(@ForcedDampedMassSpring_ODE,tspan,[x0 v0 1],options,b,k,beta,fw);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function dxdt = ForcedDampedMassSpring_ODE(t,x,b,k,beta,fw)

%One line for each state:
dxdt = [ 
        x(2)
         -b*x(2)-k^2*x(1) + beta*cos(x(3)*fw);
         1
    ];
